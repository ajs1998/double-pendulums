...
fn main_distance(
    ...
    let x = (f32(corner.x) + f32(pixel_index % grid_size)) / f32(grid_size) * 2.0 - 1.0;
    let y = (f32(corner.y) + f32(pixel_index / grid_size)) / f32(grid_size) * 2.0 - 1.0;
    let pixel = pixels[pixel_index];

    let color_index = clamp(u32(pixel.distance * 255), 0, 255);
